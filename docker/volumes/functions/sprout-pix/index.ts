...
function buildOrderCode() {
  ...
  return `MF-SPROUT-${stamp}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
}
...
async function buildCardCheckoutConfig(admin: ReturnType<typeof createAdminClient>, totalCents: number) {
  ...
  try {
    ...
    console.error('[sprout-pix] Appmax card config unavailable:', error);
...
async function syncMetaPurchaseForOrder(
  ...
  try {
    ...
    console.error('[sprout-pix] Meta purchase sync failed:', message);
...
async function notifyPixSelection(orderCode: string) {
  ...
  const n8nWebhookUrl = Deno.env.get('SPROUT_N8N_PIX_WEBHOOK_URL') || Deno.env.get('N8N_PIX_WEBHOOK_URL');
  if (!n8nWebhookUrl) {
    await admin
      .from('sprout_orders')
      .update({
...
async function createPixCharge(input: CreateInput, requestContext: RequestContext) {
  ...
  const cardConfigPromise = buildCardCheckoutConfig(admin, totalCents).catch((error) => {
    console.error('[sprout-pix] Appmax card config failed:', error);
    return buildCardDisabledConfig('O cartão está temporariamente indisponível. Tente novamente em instantes.');
  });
    ...
    const createPayload = await abacatePayRequest('/v2/transparents/create', {
      ...
      body: JSON.stringify({
        ...
        data: {
          ...
          metadata: {
            source: 'sprout-discovery',
            sproutOrderId: String(order.id),
            sproutOrderCode: String(order.order_code),
...
async function checkPixStatus(pixId: string) {
  ...
  try {
    ...
    console.warn(`[sprout-pix] Falling back to legacy PIX status endpoint for ${pixId}:`, error);
  ...
  if (mapped.paymentStatus === 'paid' && currentOrder) {
    if (currentOrder.payment_status !== 'paid') {
      ...
      const n8nPaidUrl = Deno.env.get('SPROUT_N8N_PIX_PAID_WEBHOOK_URL') || Deno.env.get('N8N_PIX_PAID_WEBHOOK_URL');
      if (n8nPaidUrl) {
        fetch(n8nPaidUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        }).catch((err) => console.error('[sprout-pix] N8N paid webhook error:', err));
      }
    }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!isSafe) console.error('[sprout-pix] Unhandled error:', message);
