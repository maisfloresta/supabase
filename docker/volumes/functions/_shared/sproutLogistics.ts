...
const DEFAULT_SPROUT_LOGISTICS_WEBHOOK_URL = 'https://n8n.maisfloresta.cloud/webhook/sprout-logistica-pedido-pago';
...
async function finalizeQuizLogisticsSend(
  ...
  if (error) {
    console.error('[sprout-logistics] finalize failed:', error.message);
  }
}
...
export async function syncQuizLogisticsForOrder(
  ...
  const logisticsUrl =
    Deno.env.get('SPROUT_N8N_LOGISTICS_WEBHOOK_URL')?.trim() ||
    Deno.env.get('N8N_QUIZ_LOGISTICS_WEBHOOK_URL')?.trim() ||
    DEFAULT_SPROUT_LOGISTICS_WEBHOOK_URL;
  ...
  try {
    ...
    console.error('[sprout-logistics] sync failed:', message);
