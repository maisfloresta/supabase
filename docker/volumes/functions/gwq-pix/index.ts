...
// ── Table names (GWQ-specific) ──
const ORDERS_TABLE = 'gwq_orders';
...
function buildOrderCode() {
  ...
  return `GWQ-${stamp}-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
}
...
function summarizeAppmaxLogPayload(payload: unknown) {
  ...
  return {
    error: firstString([
      payload.error, payload.error_description, payload.message,
      isRecord(payload.errors) ? payload.errors.message : null,
    ]),
    status: firstString([
      payload.status, isRecord(payload.data) ? payload.data.status : null,
    ]),
    code: firstString([
      payload.code, isRecord(payload.error) ? (payload.error as Record<string, unknown>).code : null,
    ]),
  };
}
...
function buildInstallmentLabel(installments: number, installmentCents: number, totalCents: number) {
  if (installments === 1) return `1x de ${formatPrice(totalCents)} à vista`;
...
// ─── Product catalog (GWQ prices — matches grow-with-quiz UpsellScreen) ───
...
const CATALOG: Record<string, { id: number; name: string; options: { label: string; subtitle: string; priceCents: number }[] }> = {
  mix: {
    ...
    options: [
      { label: 'Inicial', subtitle: '50 sementes', priceCents: 3990 },
      { label: 'Popular', subtitle: '100 sementes', priceCents: 6990 },
      { label: 'Completo', subtitle: '200 sementes', priceCents: 9990 },
    ],
  },
  bandeja: {
    id: 2,
    ...
    options: [
      { label: '1 Bandeja', subtitle: 'Para até 40 sementes', priceCents: 2990 },
      { label: '2 Bandejas', subtitle: 'Para até 80 sementes', priceCents: 3990 },
      { label: '3 Bandejas', subtitle: 'Para até 120 sementes', priceCents: 4890 },
    ],
  },
  fertilizante: {
    id: 3,
    ...
    options: [
      { label: '1 Kit', subtitle: 'Para até 100 mudas', priceCents: 1700 },
      { label: '2 Kits', subtitle: 'Para até 200 mudas', priceCents: 2700 },
      { label: '3 Kits', subtitle: 'Para até 300 mudas', priceCents: 3400 },
    ],
  },
};

const GIFT_THRESHOLD_CENTS = 12000;
...
async function quoteAppmaxInstallments(_accessToken: string, totalCents: number) {
...
async function buildCardCheckoutConfig(admin: ReturnType<typeof createAdminClient>, totalCents: number) {
  ...
  try {
    ...
    console.error('[gwq-pix] Appmax card config unavailable:', error);
...
async function syncMetaPurchaseForOrder(
  ...
  if (existingMeta.purchaseSentAt) return;
  ...
  try {
    ...
    console.error('[gwq-pix] Meta purchase sync failed:', message);
...
async function notifyPixSelection(orderCode: string) {
  ...
  if (pixSelection.webhookSentAt) {
    return { notified: false, alreadyNotified: true, orderCode: order.order_code };
  }
  ...
  const n8nWebhookUrl = Deno.env.get('GWQ_N8N_PIX_WEBHOOK_URL') || Deno.env.get('N8N_PIX_WEBHOOK_URL');
  if (!n8nWebhookUrl) {
    await admin
      .from(ORDERS_TABLE)
      .update({ payment_method: 'pix', provider_response: mergedProviderResponse })
      .eq('id', order.id);
    return { notified: false, alreadyNotified: false, orderCode: order.order_code };
  }
  ...
  try {
    ...
    return { notified: true, alreadyNotified: false, orderCode: order.order_code };
  } catch (error) {
...
async function createPixCharge(input: CreateInput, requestContext: RequestContext) {
  ...
  const cardConfigPromise = buildCardCheckoutConfig(admin, totalCents).catch((error) => {
    console.error('[gwq-pix] Appmax card config failed:', error);
    return buildCardDisabledConfig('O cartão está temporariamente indisponível. Tente novamente em instantes.');
  });
  ...
  try {
    ...
    notifyPixSelection(orderCode).catch((err) => {
      console.error('[gwq-pix] Background PIX notification failed:', err);
    });
...
async function checkPixStatus(pixId: string) {
  ...
  try {
    statusPayload = await abacatePayRequest(`/v2/transparents/get?${query.toString()}`, { method: 'GET' });
  } catch (error) {
    console.warn(`[gwq-pix] Falling back to legacy PIX status endpoint for ${pixId}:`, error);
    statusPayload = await abacatePayRequest(`/v1/pixQrCode/check?${query.toString()}`, { method: 'GET' });
  }
  ...
  if (mapped.paymentStatus === 'paid' && currentOrder) {
    if (currentOrder.payment_status !== 'paid') {
      ...
      const n8nPaidUrl = Deno.env.get('GWQ_N8N_PIX_PAID_WEBHOOK_URL') || Deno.env.get('N8N_PIX_PAID_WEBHOOK_URL');
      if (n8nPaidUrl) {
        fetch(n8nPaidUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
        ...
        }).catch((err) => console.error('[gwq-pix] N8N paid webhook error:', err));
      }
    }
...
async function processCardPayment(input: CardPaymentInput) {
  ...
  if (!input.orderCode.trim()) throw new Error('Pedido inválido.');
  ...
  if (!input.holderName.trim()) throw new Error('Nome do titular é obrigatório.');

  const holderDocumentNumber = digitsOnly(input.holderDocumentNumber);
  if (holderDocumentNumber.length !== 11) throw new Error('CPF do titular inválido.');
  ...
  const { data: order, error: orderError } = await admin
    .from(ORDERS_TABLE)
    .select(`
      id, order_code, customer_name, customer_phone, customer_cpf,
      selected_color, total_cents, freight_cents, free_shipping,
      has_gift, shipping_cep, shipping_street, shipping_number,
      shipping_complement, shipping_neighborhood, shipping_city,
      shipping_state, items, payment_status, provider_response,
  ...
  if (order.payment_status === 'paid') {
    await syncQuizLogisticsForOrder(admin, {
      ...
      customer_phone: order.customer_phone, customer_cpf: order.customer_cpf,
      selected_color: order.selected_color, items: order.items,
      total_cents: Number(order.total_cents), freight_cents: Number(order.freight_cents ?? 0),
      free_shipping: order.free_shipping, has_gift: order.has_gift,
      shipping_cep: order.shipping_cep, shipping_street: order.shipping_street,
      shipping_number: order.shipping_number, shipping_complement: order.shipping_complement,
      shipping_neighborhood: order.shipping_neighborhood, shipping_city: order.shipping_city,
      shipping_state: order.shipping_state, provider_response: order.provider_response,
      logistics_webhook_sent_at: order.logistics_webhook_sent_at,
    }, { source: 'gwq-pix:card-existing' });
  ...
  if (!customerId) throw new Error('A Appmax não retornou o customer_id do cliente.');
  ...
  const orderPayload = await appmaxApiRequest(
    '/v1/orders',
    {
      method: 'POST',
      body: JSON.stringify({
        customer_id: customerId, products_value: chargedProductsValueCents,
        discount_value: 0, shipping_value: freightCents, products: appmaxProducts,
      }),
    },
    accessToken,
  );
  ...
  if (!appmaxOrderId) throw new Error('A Appmax não retornou o order_id do pedido.');
  ...
  const paymentPayload = await appmaxApiRequest(
    '/v1/payments/credit-card',
    {
      method: 'POST',
      body: JSON.stringify({
        order_id: appmaxOrderId, customer_id: customerId,
        payment_data: {
          credit_card: {
            token: cardToken,
  ...
  const providerResponse = mergeProviderResponse(order.provider_response, {
    ...
    appmax: {
      ...
      customer: customerPayload, order: orderPayload,
      payment: paymentPayload, payment_id: appmaxPaymentId,
      status: appmaxStatus,
    },
  });
  ...
  if (isPaid) {
    await syncMetaPurchaseForOrder(admin, {
      ...
      customer_cpf: order.customer_cpf, total_cents: chargedTotalCents,
      shipping_cep: order.shipping_cep, shipping_city: order.shipping_city,
      shipping_state: order.shipping_state, provider_response: providerResponse,
    ...
    if (n8nPaidUrl) {
      fetch(n8nPaidUrl, {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          event: 'payment_paid', source: 'gwq',
          orderCode: order.order_code, customerName: order.customer_name,
          customerPhone: order.customer_phone, totalCents: chargedTotalCents,
          baseTotalCents, installmentFeeCents, paidAt: new Date().toISOString(),
        }),
      }).catch((err) => console.error('[gwq-pix] N8N paid webhook error:', err));
    }
    ...
    await syncQuizLogisticsForOrder(admin, {
      ...
      customer_phone: order.customer_phone, customer_cpf: order.customer_cpf,
      selected_color: order.selected_color, items: order.items,
      total_cents: chargedTotalCents, freight_cents: freightCents,
      free_shipping: order.free_shipping, has_gift: order.has_gift,
      shipping_cep: order.shipping_cep, shipping_street: order.shipping_street,
      shipping_number: order.shipping_number, shipping_complement: order.shipping_complement,
      shipping_neighborhood: order.shipping_neighborhood, shipping_city: order.shipping_city,
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'pix_selected') {
      ...
      if (!orderCode) return errorResponse('Pedido inválido.', 400);
    ...
    const safeMessages = [
      ...
      'A Appmax não retornou o token do cartão.',
      'A Appmax não retornou o customer_id do cliente.',
      'A Appmax não retornou o order_id do pedido.',
      'A instalação Appmax ainda não foi concluída. Finalize a instalação do aplicativo e tente novamente.',
    ...
    if (!isSafe) console.error('[gwq-pix] Unhandled error:', message);
